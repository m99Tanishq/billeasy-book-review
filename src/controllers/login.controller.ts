...
export const loginController = asyncHandler(async (req, res) => {
  ...
  return res.json({ success: true, email: user.email, token, name: user.name });
});

export const signUpController = asyncHandler(async (req, res) => {
  const { email, password, name }: SignUpSchema = req.body;
  if (!password || !email || !name) throw new AppError(ERRORS.MISSING_VALUES.message);
  ...
  const newUser = await createUserService({email, hashedPassword, name, clientId: 1});
  return res.json({ success: true, email: newUser.email, name: newUser.name });
});
